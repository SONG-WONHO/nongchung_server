    ...
    if(!token){
        ...
        if(decoded==-1){
            ...
            let getBookmarkListQuery =
                `SELECT idx, price, star, period, name, 1 AS isBooked, addr, img FROM (SELECT idx, name, price, star, period, addr, addrIdx, img
                FROM 
                ...
                ON nh.farmIdx = farm.farmIdx) AS nh, bookmark WHERE userIdx = ? AND bookmark.nhIdx = nh.idx;`;
